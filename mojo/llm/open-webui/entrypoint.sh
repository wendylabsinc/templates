...
# service-name URL (http://max-serve:9011) is not usable from Open WebUI. Avoid
...
# max-serve service publishes 9011 on the shared device network stack, making
# loopback the stable in-app route.
if [[ -n "${WENDY_HOSTNAME:-}" ]]; then
  case "${OPENAI_API_BASE_URL:-}" in
    "http://max-serve:9011/v1"|"http://${WENDY_HOSTNAME}:9011/v1")
      export OPENAI_API_BASE_URL="http://127.0.0.1:9011/v1"
