...
# off to the Pipecat app. In this MAX variant the local LLM is the
# max-serve service in the same app group (see ../max-serve/), so this
# container stays daemon-free; the "ollama" provider path points at
# max serve via LLM_BASE_URL and its liveness is watched through
# OLLAMA_HEALTH_URL (both set in the Dockerfile).
