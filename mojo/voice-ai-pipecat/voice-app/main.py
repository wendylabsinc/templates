...
# MAX variant: the Dockerfile points these at the max-serve service in
# this app group (LLM_PROVIDER=ollama is the OpenAI-compatible local
# path; max serve speaks the same protocol). The python sibling's
# cloud defaults remain the fallback so this file stays a close copy.
DEFAULT_LLM_PROVIDER = os.environ.get("LLM_PROVIDER", "google")
...
LLM_PROVIDERS: dict[str, list[str]] = {
    ...
    # On-device LLM. In this MAX variant the "ollama" provider key is
    # the OpenAI-compatible local path, served by the max-serve service
    # in this app group. The Ollama-style names below stay listed for
    # users who point llm_base_url at a real Ollama daemon instead.
    "ollama": [
        "qwen2.5:3b",
        "llama3.2:3b",
        "qwen2.5:7b",
...
# max serve hosts exactly one model per process — the MAX_MODEL template
# variable, exported as LLM_MODEL by the Dockerfile. Put it first in the
# local picker so the settings drawer reflects what is actually running.
_local_llm_model = os.environ.get("LLM_MODEL", "")
if _local_llm_model and _local_llm_model not in LLM_PROVIDERS["ollama"]:
    LLM_PROVIDERS["ollama"].insert(0, _local_llm_model)
...
# here. The Dockerfile sets LLM_BASE_URL to the max-serve service's
# OpenAI endpoint; the Ollama URL stays the fallback for bare runs.
        ...
        try:
            ...
            try:
                ...
                _ollama_health_error = (
                    f"Local LLM server not reachable at {OLLAMA_HEALTH_URL}: "
                    f"{exc}. Local LLM turns will fail until it is back up."
                )
        except asyncio.CancelledError:
            raise
        except Exception:
