...
Node.js 22, GStreamer, `arecord`, and `aplay`.
...
| `PORT` | `5004` | HTTP/WebSocket listener, readiness probe, and UI port |

## How it works

`src/index.ts` discovers ALSA devices, manages the GStreamer capture process,
broadcasts PCM frames, and starts a separate GStreamer process for sample
...
- Change the PCM caps in `src/index.ts` and the browser decoder in `index.html`
  together when changing format, channels, or sample rate.
- Add Express routes or controls and update `wendy.json` for new hardware.
