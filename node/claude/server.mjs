...
const CLAUDE_HOME_DIR = "/home/claude";
const HISTORY_PATH = path.join(STATE_DIR, "claude-history.ndjson");
const CONSOLE_TOKEN = (process.env.CONSOLE_TOKEN || "").trim();
...
function isAuthorized(req, url) {
  ...
  return tokenFrom(req, url) === CONSOLE_TOKEN;
}
...
function requireAuth(req, url, res) {
  ...
  sendJson(res, 401, {
    error: "unauthorized",
    message: "A valid console access token is required.",
  });
  return false;
}
...
function runClaude(userPrompt) {
  ...
  const child = spawn(
    "/bin/sh",
    ["-lc", `${CLAUDE_COMMAND} "$@"`, "claude-console", prompt],
    {
      cwd: WORKSPACE_DIR,
      detached: true,
      env: {
...
async function handleApi(req, res, url) {
  if (url.pathname === "/api/config" && req.method === "GET") {
    sendJson(res, 200, {
      authRequired: Boolean(CONSOLE_TOKEN),
      appName: "Claude Console",
    });
    return;
  }
