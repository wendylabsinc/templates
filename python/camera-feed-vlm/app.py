#!/usr/bin/env python3
"""Live VLM camera chat: GStreamer (or OpenCV fallback) → MJPEG over WebSocket,
plus a `POST /ask` endpoint that runs a vision-language model on the current
frame and streams back an answer.

The video stream and the VLM are fully decoupled: frames flow to clients at
camera rate over `/stream`, while each chat question grabs the latest frame and
runs one (slow) `moondream2` query — so inference never stalls the live feed.

Env overrides: CAMERA_BACKEND=opencv|gstreamer, VLM_MODEL=<hf-id>,
VLM_REVISION=<rev>, VLM_MAX_TOKENS=<n>.
"""
...
if not _HAS_CUDA:
    # Hide CUDA from torch on CPU-only devices so the model loads on CPU.
    os.environ.setdefault("CUDA_VISIBLE_DEVICES", "")

# RPi5 browns out under GStreamer when USB-powered; OpenCV idles lighter.
...
logger.info("Platform: %s (device=%s, gpu=%s), CUDA: %s, capture: %s",
            ...
            _HAS_CUDA,
            "opencv" if _FORCE_OPENCV else "gstreamer")

_VLM_MODEL_ID = os.environ.get("VLM_MODEL", "vikhyatk/moondream2")
_VLM_REVISION = os.environ.get("VLM_REVISION", "2025-06-21")
_VLM_MAX_TOKENS = int(os.environ.get("VLM_MAX_TOKENS", "256"))
...
def _get_model():
    """Lazily load the VLM once, behind a lock. Warmed at startup so the first
    question isn't blocked by the multi-second torch + weights load."""
    global _model
    ...
    with _model_lock:
        ...
        dtype = torch.float16 if _HAS_CUDA else torch.float32
        logger.info("Loading VLM %s (rev %s) on %s...", _VLM_MODEL_ID, _VLM_REVISION, device)
        ...
        try:
            ...
            pass
        logger.info("VLM ready: %s on %s", _VLM_MODEL_ID, device)
        _model = m
        return _model
...
class VLMCamera:
    """Camera frames stream to clients at camera rate over `/stream`. The VLM is
    never in that path: `POST /ask` reads the latest frame (`latest_frame_jpeg`)
    and runs one query, so slow inference never stalls the live feed."""
    ...
    def _start_pipeline(self, device_id: str | None = None) -> Gst.Pipeline | None:
        ...
        if _HAS_CUDA:
            ...
            # CPU: passthrough first — skip 30 fps decode/re-encode.
            pipelines = [
                f"{src} ! image/jpeg ! {appsink}",
                f"{src} ! image/jpeg,width=640,height=480 ! {appsink}",
    ...
    def latest_frame_jpeg(self) -> bytes | None:
        """Latest JPEG frame, read non-destructively (questions don't consume it)."""
        with self._lock:
...
# Serialize VLM queries — the Orin Nano runs one at a time and moondream's
# query() is not safe to call concurrently.
_ask_lock = asyncio.Lock()

# Warm the model at startup so the first question isn't stalled by the
# multi-second weights load.
...
async def ask(request: Request):
    """Run the VLM on the latest camera frame and stream back the answer."""
    try:
    ...
    raw = camera.latest_frame_jpeg()
    if not raw:
        return JSONResponse(status_code=409, content={"error": "no frame yet — wait for the live feed"})

    try:
        image = Image.open(io.BytesIO(raw)).convert("RGB")
    except Exception:
        return JSONResponse(status_code=409, content={"error": "could not decode the current frame"})

    async def token_stream():
        # One query at a time; stream tokens from the worker thread via a queue.
        async with _ask_lock:
            ...
            def run():
                try:
                    # Try richest call first, degrading on signature mismatch so this
                    # works across moondream revisions (and other VLMs): streaming +
                    # token cap → streaming → plain.
                    result = None
                    for kw in (
                        {"stream": True, "settings": {"max_tokens": _VLM_MAX_TOKENS}},
                        {"stream": True},
                        {},
                    ):
                        try:
                            result = model.query(image, question, **kw)
                            break
                        except TypeError:
                            continue
                    if result is None:
                        raise RuntimeError("VLM query signature not supported")
                    answer = result["answer"] if isinstance(result, dict) else result
                    if isinstance(answer, str):
                        loop.call_soon_threadsafe(tokens.put_nowait, answer)
                    else:
                        for tok in answer:
                            loop.call_soon_threadsafe(tokens.put_nowait, tok)
                except Exception as e:
                    logger.error("VLM query failed: %s", e, exc_info=True)
                    loop.call_soon_threadsafe(tokens.put_nowait, f"[error: {e}]")
                finally:
                    loop.call_soon_threadsafe(tokens.put_nowait, sentinel)
...
async def debug_info():
    ...
    return JSONResponse(
        content={
            ...
            "has_frame": camera.latest_frame_jpeg() is not None,
        }
    )
