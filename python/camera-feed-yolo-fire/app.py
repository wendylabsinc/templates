#!/usr/bin/env python3
"""YOLOv8 fire/smoke detection webcam server: GStreamer (or OpenCV fallback) → YOLO → annotated MJPEG over WebSocket.

Uses a custom fire.pt model (classes: fire, other, smoke), baked into the image.
Env overrides: CAMERA_BACKEND=opencv|gstreamer, YOLO_MAX_FPS=<n>, FIRE_MODEL=<path>.
"""
...
class YOLOCamera:
    ...
    def __init__(self):
        ...
        # Fire/smoke detection works best at a higher threshold to suppress
        # false positives on warm-coloured backgrounds (default 0.6).
