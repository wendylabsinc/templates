...
_TRT_IMGSZ = int(os.environ.get("YOLO_IMGSZ", "640"))
...
def _get_model():
    ...
    with _model_lock:
        ...
        if _USE_TRT:
            ...
            engine_path = os.environ.get("YOLO_ENGINE_PATH", "/app/yolov8n.engine")
            logger.info("Loading YOLOv8n via TensorRT (engine cache: %s)...", engine_path)
            m = TRTYolo("yolov8n.onnx", engine_path, imgsz=_TRT_IMGSZ)
            logger.info("YOLOv8n (TensorRT) ready — %d COCO classes", len(m.names))
        else:
            # CPU (or non-Jetson GPU): Ultralytics. Prefer the .pt (torch) weights.
            if YOLO is None:
                raise RuntimeError("ultralytics unavailable and YOLO_BACKEND != trt")
            model_path = "yolov8n.pt" if Path("yolov8n.pt").exists() else "yolov8n.onnx"
            logger.info("Loading YOLOv8n via Ultralytics (CUDA: %s)...", _HAS_CUDA)
            m = YOLO(model_path)
