...
A Python and React starter for WendyOS. It serves a device dashboard, a
...
- Network access during the first build for Python and frontend packages
...
|---|---:|---|
| `APP_ID` | required | Application and persistent-volume name |
| `PORT` | `3001` | HTTP/WebSocket listener, readiness probe, and UI port |
...
- `app/` contains the FastAPI application, route modules, SQLite helper, device
  discovery, and GStreamer sink.
- `frontend/src/` contains the React pages and navigation.
- `Dockerfile` builds the frontend and Python runtime, then serves the compiled
  UI through FastAPI.
...
Add a router under `app/routes/` and include it in `app/__init__.py`. Add a page
under `frontend/src/pages/` and register it in the frontend router. To develop
the UI shell, run `npm install` and `npm run dev` in `frontend/`; add a Vite
proxy when connecting it to a separately running backend.

Change `app/lib/db.py` with any schema changes. Update `wendy.json` when a new
