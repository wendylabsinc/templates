...
A Rust audio device demo for WendyOS. It streams microphone samples to a browser
...
Rust, GStreamer development packages, `arecord`, and `aplay`.
...
| `PORT` | `4004` | HTTP/WebSocket listener, readiness probe, and UI port |

## How it works

`src/main.rs` discovers ALSA devices, starts and manages the GStreamer capture
process for the service lifetime, broadcasts PCM frames to connected clients,
and starts a separate GStreamer process for sample playback.
...
- Change the PCM caps in `src/main.rs` and the browser decoder in `index.html`
  together when changing format, channels, or sample rate.
- Add Axum routes or controls and update `wendy.json` for new hardware.

After installing the native dependencies, run `cargo run` for local work.
