...
A Rust camera viewer with YOLOv8 object detection. It runs ONNX Runtime on the
...
- Network access during the first build to fetch crates and export the YOLOv8n
  ONNX model
...
| `PORT` | `4006` | HTTP/WebSocket listener, readiness probe, and UI port |
...
- `src/main.rs` captures MJPEG with GStreamer, preprocesses 640-pixel
  letterboxed input, runs ONNX Runtime, filters detections, and publishes COCO
  labels.
...
- Replace the model export in `Dockerfile`, then update tensor decoding and the
  label list in `src/main.rs` to match the new model.
- Change capture settings in the GStreamer pipeline builder.
- Extend the WebSocket metadata and its rendering in `index.html`.

Use `cargo run` for local Rust work after installing GStreamer, TurboJPEG, and
a compatible ONNX Runtime library.
