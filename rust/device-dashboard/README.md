...
A Rust and React starter for WendyOS. It serves a device dashboard, a persistent
...
- Network access during the first build for Rust and frontend packages
...
| `PORT` | `4001` | HTTP/WebSocket listener, readiness probe, and UI port |
...
- `src/main.rs` implements the Axum API, SQLite store, device discovery, and
  GStreamer camera/audio pipelines.
- `frontend/src/` contains the React pages and navigation.
- `Dockerfile` builds the frontend and Rust server, then serves the compiled UI.
...
Add handlers and routes in `src/main.rs`, splitting modules out as the backend
grows. Add a page under `frontend/src/pages/` and register it in the frontend
